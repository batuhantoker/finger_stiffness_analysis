...
rms_flex, mav_flex, iav_flex, var_flex, wl_flex, mf_flex, pf_flex, mp_flex, tp_flex, sm_flex, msf_flex,rms_ext, mav_ext, iav_ext, var_ext, wl_ext, mf_ext, pf_ext, mp_ext, tp_ext, sm_ext, msf_ext, movement_id, force = list(map(features.get, ['rms_flex', 'mav_flex', 'iav_flex', 'var_flex', 'wl_flex', 'mf_flex', 'pf_flex', 'mp_flex', 'tp_flex', 'sm_flex', 'msf_flex','rms_ext', 'mav_ext', 'iav_ext', 'var_ext', 'wl_ext', 'mf_ext', 'pf_ext', 'mp_ext', 'tp_ext', 'sm_ext', 'msf_ext', 'movement_id','force']) )
movement_id=np.asarray(movement_id)
valid_classes=np.r_[np.array([i for i, v in enumerate(movement_id) if v.is_integer()])]
...
print(movement_id)
X_train, X_test, y_train, y_test = train_test_split(rms_features, movement_id, test_size=0.25, random_state=42)
