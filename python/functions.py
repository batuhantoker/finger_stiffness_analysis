...
def vaf(real,estimate):
    return 100*(1-(np.var(real-estimate)/np.var(estimate)))#100*(1-(np.sum((real-estimate)**2)/np.sum((estimate**2))))
...
def force_window(splitted_data):
    ...
    for i in range(len(splitted_data)):
        if i==0:
            new_data.append(np.concatenate((splitted_data[i], splitted_data[i+1], splitted_data[i+2])))
        if i!=0 and i!=len(splitted_data)-2 and i!=len(splitted_data)-1:
            new_data.append(np.concatenate((splitted_data[i-1], splitted_data[i], splitted_data[i + 1])))
        if i==len(splitted_data) or i==len(splitted_data)-1 or i==len(splitted_data)-2:
            new_data.append(np.concatenate((splitted_data[i], splitted_data[i-1], splitted_data[i -2])))
...
def evaluate_regression_metrics(y_pred, y_true,index):
    ...
    metrics = {'R2': r2*100, 'MAE': mae, 'MSE': mse, 'RMSEP': rmsep, 'vaf': vaf_value, 'RMSE': rmsep, 'nRMSE1': nrmse_value1*100, 'nRMSE2': nrmse_value2*100}
